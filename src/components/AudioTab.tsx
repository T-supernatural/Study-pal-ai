...
export const AudioTab: React.FC = () => {
  const { notes, activeNote, setActiveNote } = useApp();
  ...
  return (
    <div class="space-y-6 pb-24">
      ...
      <div class="text-center space-y-1">
        <h2 class="font-display text-lg font-bold text-royal">Audio Notes Player</h2>
        <p class="text-xs text-powder">Listen to AI synthesized lectures & summaries</p>
      </div>

      {selectedNote ? (
        <div class="space-y-6">
          {/* 2. Audio Card Plate */}
          <NeumorphicCard class="space-y-6 relative overflow-hidden border border-white">
            <div class="absolute -top-12 -left-12 w-24 h-24 bg-[#A9C0E0]/10 rounded-full" />
            
            {/* Subject details & lesson meta */}
            <div class="text-center space-y-1 relative z-10">
              <span class="text-[9px] font-mono font-bold bg-blue-50 text-blue-600 border border-blue-100 px-2 py-0.5 rounded-full uppercase tracking-wider">
                {selectedNote.category}
              </span>
              <h3 class="font-display text-md font-extrabold text-royal leading-tight pt-1 truncate">
                {selectedNote.title}
              </h3>
              <p class="text-[10px] text-powder font-medium">Lecturer Mode • AI Synthesizer</p>
            </div>

            {/* Waveform Visualization Plate (bouncing bars) */}
            <div class="h-20 bg-blue-50/40 rounded-2xl flex items-center justify-center space-x-0.5 px-6 shadow-inner border border-[#A9C0E0]/15">
              {waveHeights.map((h, i) => (
                <div
                  key={i}
                  class={`
            ...
            <div class="space-y-1">
              <div class="flex items-center justify-between text-[10px] font-mono font-bold text-royal/80">
                <span>{currentTime}</span>
                <span>{duration}</span>
              </div>
              <div class="w-full bg-[#A9C0E0]/20 h-2 rounded-full overflow-hidden">
                <div 
            ...
            <div class="flex items-center justify-center space-x-6">
              <button class="w-10 h-10 rounded-full neumorphic-card flex items-center justify-center text-royal/60 cursor-not-allowed">
              ...
              <button
                ...
                class="w-16 h-16 rounded-full bg-gradient-to-tr from-royal to-blue-600 text-white flex items-center justify-center shadow-lg soft-glow-blue hover:from-blue-600 hover:to-royal"
              >
                {isPlaying ? (
                  <Pause class="w-7 h-7 fill-white stroke-none" />
                ) : (
                  <Play class="w-7 h-7 fill-white stroke-none ml-1" />
                )}
              </button>
            ...
            <div class="grid grid-cols-2 gap-3 pt-2">
              <button
                ...
                class="py-2.5 rounded-full neumorphic-card text-[11px] font-bold text-royal flex items-center justify-center space-x-1.5"
              >
                <span>{playbackSpeed.toFixed(2)}x</span>
                <span class="text-[9px] text-powder uppercase font-mono tracking-wider font-bold">Speed</span>
              </button>
              ...
              <button
                ...
                class="py-2.5 rounded-full neumorphic-card text-[11px] font-bold text-royal flex items-center justify-center space-x-1.5"
              >
                <Volume2 class="w-3.5 h-3.5 text-royal" />
          ...
          <div class="space-y-3">
            <h4 class="font-display text-sm font-bold text-royal pl-1">Available Audios</h4>
                ...
                return (
                  ...
                  >
                    <div class="flex items-center space-x-3 max-w-[80%]">
                      <div class={`w-9 h-9 rounded-xl flex items-center justify-center shadow-inner ${
                        isSelected ? "bg-royal text-white" : "bg-ice text-royal"
                      }`}>
                        <Headphones class="w-4.5 h-4.5" />
                      </div>
                      <div class="space-y-0.5">
                        <p class={`text-xs font-bold leading-tight ${isSelected ? "text-royal" : "text-royal/80"}`}>
                          {note.title}
                        </p>
                        <span class="text-[10px] text-powder font-medium">{note.category}</span>
                      </div>
                    </div>
                    {isSelected && isPlaying && (
                      <span class="flex space-x-0.5 items-end h-3 pr-2">
                        <span class="w-1 bg-royal h-full rounded-full animate-[bounce_0.6s_infinite_100ms]" />
                        <span class="w-1 bg-royal h-2 rounded-full animate-[bounce_0.6s_infinite_200ms]" />
                        <span class="w-1 bg-royal h-full rounded-full animate-[bounce_0.6s_infinite_300ms]" />
                      </span>
                    )}
                  </div>
                );
